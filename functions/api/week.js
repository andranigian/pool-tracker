  ...
  try {
    ...
    try {
      const res = await env.PICKS.prepare(
        `SELECT id, sheet_number, sheet_label, sheet_label_dog, market, favorite, underdog, spread, status, winner_side, fav_score, dog_score FROM games WHERE week_id = ? ORDER BY sheet_number`
      ).bind(week.id).all();
      games = res.results || [];
    } catch (e) {
